...
var setIndex = function setIndex(id) {
...
var updateGame = function updateGame(i) {
  if (ui.didIWin(currentPlayer, board) || board.every(function (element) {
    return element !== '';
  })) {
  ...
  api.gameUpdate().then(ui.updateSuccess).catch(ui.updateFailure);
};

var onClick = function onClick(event) {
  var content = event.target.innerText;
  var id = event.target.id;
  var index = setIndex(id);
...
var onSignIn = function onSignIn(event) {
  ...
  }).then(ui.createSuccess).catch(ui.signInFailure);
};
...
var onShowGames = function onShowGames(event) {
  event.preventDefault();
  api.showGames().then(ui.showSuccess).catch(ui.showFailure);
};

var onFindGame = function onFindGame(event) {
  event.preventDefault();
  api.findGame().then(ui.findSuccess).catch(ui.findFailure);
};

var wipeGame = function wipeGame() {
...
var onCreatNewGame = function onCreatNewGame(event) {
  event.preventDefault();
  wipeGame();
...
var onSignOut = function onSignOut(event) {
  ...
  updateGame();
  api.signOut().then(ui.signOutSuccess).then(wipeGame()).catch(ui.signOutFailure);
};
...
var didIWin = function didIWin(currentPlayer, gameBoard) {
...
var findFailure = function findFailure(data) {
